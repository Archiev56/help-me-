...
func set_slot_data(value: SlotData) -> void:
	slot_data = value
	if slot_data == null:
		texture_rect.texture = null  # Clear texture if slot is empty
		label.text = ""  # Clear label
		return  # Exit function early
		
	if slot_data.item_data != null:  # Ensure item_data exists before accessing texture
		texture_rect.texture = slot_data.item_data.texture
	else:
		texture_rect.texture = null  # If item_data is null, clear texture
		label.text = str(slot_data.quantity) if slot_data.quantity > 0 else ""
